...
export default function CollectionsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      setIsLoading(true);
      try {
        const [collectionsData, blocksData] = await Promise.all([
          getCollections(),
          getBlocks(),
        ]);
  ...
  const handleCreate = async () => {
    ...
    try {
      const col = await createCollection(formName.trim(), formEmoji.trim() || undefined, formDescription.trim() || undefined);
  ...
  const handleMoveBlocks = async (targetCollectionId: string) => {
    ...
    try {
      ...
      setAllBlocks((prev) =>
        prev.map((b) =>
          ids.includes(b.id) ? { ...b, collectionId: realTarget } : b
        )
      );
      // 현재 뷰에서 이동된 블록 제거
      setCollectionBlocks((prev) => prev.filter((b) => !ids.includes(b.id)));
      setSelectedBlockIds(new Set());
      const targetName = targetCollectionId === UNCATEGORIZED_ID
        ? '미분류'
  ...
  // 현재 활성 컬렉션 정보
  const activeCollection = activeCollectionId === UNCATEGORIZED_ID
  ...
  if (activeCollectionId && activeCollection) {
    return (
      <div className="space-y-4">
        ...
        {isLoadingBlocks ? (
          ...
          <Card>
            <CardContent className="py-12 text-center text-[var(--color-text-secondary)]">
              ...
              <p>이 컬렉션에 블록이 없습니다.</p>
              <p className="mt-1 text-sm">이미지를 분석할 때 이 컬렉션을 선택하거나, 다른 컬렉션에서 블록을 이동하세요.</p>
            </CardContent>
          </Card>
        ) : (
            ...
            {collectionBlocks.map((block) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={cn(
                      'inline-flex items-center rounded px-2 py-0.5 text-xs font-medium text-white shrink-0',
            ...
            <div className="max-h-[300px] overflow-y-auto space-y-2">
              ...
              {activeCollectionId !== UNCATEGORIZED_ID && (
                <div
                  ...
                  className="flex items-center gap-3 p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] cursor-pointer hover:border-[var(--color-primary)] hover:bg-[var(--color-primary)]/5 transition-colors"
                >
                  <span className="text-lg">📁</span>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">미분류</span>
                </div>
              )}
              {/* 컬렉션들 */}
              {collections
                ...
                .map((col) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-[var(--color-text-primary)]">{col.name}</span>
                  </div>
                ))}
            </div>
        ...
        {/* 편집 모달 */}
        <Modal isOpen={showEditModal} onClose={() => { setShowEditModal(false); setEditingCollection(null); resetForm(); }} title="컬렉션 편집">
          <div className="space-y-4">
            <Input label="컬렉션 이름" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="예: 웨딩 사진" />
            <Input label="이모지 (선택)" value={formEmoji} onChange={(e) => setFormEmoji(e.target.value)} placeholder="예: 🎨" />
            <Input label="설명 (선택)" value={formDescription} onChange={(e) => setFormDescription(e.target.value)} placeholder="이 컬렉션에 대한 설명" />
            <div className="flex justify-end gap-3">
              <Button variant="secondary" onClick={() => { setShowEditModal(false); resetForm(); }}>취소</Button>
        ...
        {/* 삭제 확인 모달 */}
        <Modal isOpen={showDeleteModal} onClose={() => { setShowDeleteModal(false); setDeletingCollection(null); }} title="컬렉션 삭제">
          <div className="space-y-4">
            <p className="text-sm text-[var(--color-text-secondary)]">
            ...
            <div className="flex justify-end gap-3">
              <Button variant="secondary" onClick={() => { setShowDeleteModal(false); setDeletingCollection(null); }}>취소</Button>
  ...
  return (
    <div className="space-y-4">
      ...
      {collections.length === 0 && blockCounts[UNCATEGORIZED_ID] === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-[var(--color-text-secondary)]">
            <FolderOpen className="h-12 w-12 mx-auto mb-3 opacity-30" />
            <p>컬렉션이 없습니다.</p>
            <p className="mt-1 text-sm">이미지를 분석할 때 컬렉션을 만들거나, 위의 버튼으로 직접 생성하세요.</p>
          </CardContent>
        </Card>
      ) : (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); startDelete(col); }}
                ...
                {col.description && (
                  <p className="mt-2 text-xs text-[var(--color-text-secondary)] line-clamp-1">
                    {col.description}
                  </p>
                )}
              </CardContent>
      ...
      {/* 생성 모달 */}
      <Modal isOpen={showCreateModal} onClose={() => { setShowCreateModal(false); resetForm(); }} title="새 컬렉션">
        <div className="space-y-4">
          <Input label="컬렉션 이름" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="예: 웨딩 사진" />
          <Input label="이모지 (선택)" value={formEmoji} onChange={(e) => setFormEmoji(e.target.value)} placeholder="예: 💒" />
          <Input label="설명 (선택)" value={formDescription} onChange={(e) => setFormDescription(e.target.value)} placeholder="이 컬렉션에 대한 설명" />
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => { setShowCreateModal(false); resetForm(); }}>취소</Button>
